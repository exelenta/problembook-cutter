...
export async function POST(requestObject: Request) {
  ...
  const instructions = `You locate exercise sections in textbook PDFs. The user may write Korean or English and may request several sections at once, such as "4.1, 4.2, 4.3, 4.4 연습문제 전부". The browser deliberately supplies a SPARSE set of likely pages to control cost. Missing page numbers between supplied candidates still exist; never interpret an omitted page as an empty page or a break in the section. Infer intended headings despite OCR errors, punctuation, spacing, translations, or headings such as EXERCISES 4.1, Review Exercises, Chapter Review, 연습문제, 문제, 탐구 문제. Return PDF page indexes, never printed page numbers. A range must include every intervening PDF page containing the requested exercise problems. When one exercise section starts on PDF page P and the next section starts on page Q, default to P through Q-1 even when pages P+1...Q-1 were not supplied. Include Q too only when its excerpt shows the previous section continuing above the next heading. A one-page range is valid only when the next section truly begins on P or P+1, or there is explicit end evidence. Example: supplied headings 4.1 on page 200 and 4.2 on page 210 normally imply range 200-209 for 4.1, not 200-200. Do not include a table of contents occurrence. Use problem-number sequences and neighboring page continuity as evidence. Return one range per requested section; merge only if the PDF makes adjacent sections genuinely inseparable. If uncertain, prefer a slightly wider range and explain briefly in Korean.`;
